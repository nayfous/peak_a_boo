...
def indexes(y, thres=0.3, min_dist=1, max_ammount = -1):
    ...
    *thres* and *min_dist* parameters, it is possible to reduce the number of
    ...
    if peaks.size > 1 and min_dist > 1:
        ...
        if max_ammount > -1:
            values = y[peaks]
            peakindexes = []
            for i in heapq.nlargest(max_ammount, values):
                peakindexes.append(list(values).index(i))
            peaks = peaks[peakindexes]
